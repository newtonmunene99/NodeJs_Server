...
newcon.connect(err => {
    if (err) {
        ...
        newcon.query('CREATE DATABASE IF NOT EXISTS ' + database + '', (err, result) => {
            if (err) {
                console.log(err);
            }
            else {
                    ...
                    if (usrresult[0]) {
                        ...
                        bcrypt.genSalt(10, (err, salt) => {
                            bcrypt.hash(req.query.password, salt, (err, hash) => {
                                var addUser = "INSERT INTO users (name, username, email, password) VALUES ('" +
                                    req.query.name +
                                    "', '" +
                                    req.query.username +
                                    "', '" +
        ...
        if (err) {
            ...
            if (result[0]) {
                ...
                bcrypt.compare(req.query.oldpassword, hash, (err, respwd) => {
                    if (respwd === true) {
                        bcrypt.genSalt(10, (err, salt) => {
                            bcrypt.hash(req.query.newpassword, salt, (err, newhash) => {
                                var updatePWD = 'UPDATE users SET password = ' +
        ...
        if (err) {
            ...
            if (result[0]) {
                ...
                bcrypt.compare(req.body.oldpassword, hash, (err, respwd) => {
                    if (respwd === true) {
                        bcrypt.genSalt(10, (err, salt) => {
                            bcrypt.hash(req.body.newpassword, salt, (err, newhash) => {
                                var updatePWD = 'UPDATE users SET password = ' +
                                    mysql.escape(newhash) +
                                    '';
                                con.query(updatePWD, (err, result) => {
